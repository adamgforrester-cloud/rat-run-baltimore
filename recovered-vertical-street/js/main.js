...
const comboPill = comboEl.closest(".hud-pill");
...
const game = new RatRunGame(canvas, {
  onScore(score,combo) {
    ...
    if (combo>0) {
      comboPill.classList.remove("combo-pop","combo-hot");
      void comboPill.offsetWidth;
      comboPill.classList.add("combo-pop");
      if (combo>=6) comboPill.classList.add("combo-hot");
    } else {
      comboPill.classList.remove("combo-hot");
    }
  },
  onTime(time) {
